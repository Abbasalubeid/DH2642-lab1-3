...
function compareIngredientsCB(ingredientA, ingredientB){
    ...
    if (ingredientA.aisle < ingredientB.aisle)
        ...
        return 1;
    // If they have the same aisle, check the name
...
function isKnownTypeCB(type){
    ...
    //It either undefined (falsy) or truthy
...
function dishType(dish){
    ...
    // If the array has a falsy value
    if(!dish.dishTypes) 
    return "";
    // If we can find a type that is known (starter, main course or dessert)
    if (dish.dishTypes.find(isKnownTypeCB))
        return dish.dishTypes.find(isKnownTypeCB);
    // If the array had valid property but nothing that is known to us
    else return "";
...
function compareDishesCB(dishA, dishB){
    ...
    // Gets the value of each dish and compares them, if they are in order it should give a falsy values, else truthy to switch
...
function shoppingList(dishes){
    ...
    function ingredientCB(ingredient){
        if(result[ingredient.id] === undefined){  // more general: !result[ingredient.id]
            ...
            // since result[ingredient.id] is not defined, it means that the ingredient has been encountered before. // QUESTION
            // It should say "undefined" above right?
...
function menuPrice(dishesArray){
    ...
    function summationCB(elem1, elem2){
        return elem1 + elem2;
    }
    ...
    // arr.reduce(summationCB);
    return dishesArray.map(dishPriceCB).reduce(summationCB, 0);
}
